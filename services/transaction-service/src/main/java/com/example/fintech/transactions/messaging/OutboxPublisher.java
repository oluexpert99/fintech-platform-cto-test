package com.example.fintech.transactions.messaging;
...
/**
 * In-process transactional-outbox publisher. Algorithm in
 * {@code transaction-service.spec} §4.4 + §4.4.1.
 *
 * <p>Drains the per-service {@code outbox_txn} collection to Kafka. Uses
 * {@link OutboxRepository#claimPending} to lease-claim rows atomically — multiple replicas can
 * run this publisher in parallel without ever publishing the same row twice (consumer dedupe is
 * still required as the second line of defence; see ADR-0002).
 */
@Component
public class OutboxPublisher {
    ...
    public void drain() {
        ...
        log.debug("publisher {} claimed {} rows", instanceId, batch.size());
    ...
    private void publish(OutboxRecordDocument record) {
        try {
            ...
            if (newAttempts >= maxAttempts) {
                ...
                meterRegistry.counter("outbox.poisoned.total", "topic", record.getTopic()).increment();
                ...
                log.warn("Outbox send failed for row {} (attempt {}): {}", record.getId(), newAttempts, msg);
            }
        }
    }
}
